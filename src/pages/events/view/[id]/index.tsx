...
import { deleteTicketById } from 'apiSdk/tickets';
...
function EventViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<EventInterface>(
    ...
    () =>
      getEventById(id, {
        relations: ['organization', 'ticket'],
      }),
  );

  const ticketHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('ticket', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/tickets/create?event_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.ticket?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/tickets/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.status}</Td>
                            <Td>
